...
% derivatives of f wrt X, the "len" gives the length of the run: if it is
% positive, it gives the maximum number of line searches, if negative its
% absolute gives the maximum allowed number of function evaluations. You can
% (optionally) give "len" a second component, which will indicate the
...
% depending on the sign of "len") used.
%
...
%
% 1) we first enter a while loop which uses (p1) and (p2) to compute an 
% extrapolation (p3), until we have extrapolated far enough (Wolfe-Powell 
% conditions). 
%
...
% 
% 3) compute a new search direction using conjugate gradients (Polack-Ribiere 
% flavour), or revert to steepest if there was a problem in the previous 
% line-search. Return the best value so far, if two consecutive line-searches 
% fail, or whenever we run out of function evaluations or line-searches. 
% 
...
while i < abs(len),	% while not finished
  ...
  %
  % Step 1. Initial line search
  %
  while 1, % keep extrapolating as long as necessary
    x2 = 0; f2 = f0; d2 = d0; % point 2
    f3 = f0; df3 = df0;
  ...
  %
  % Step 3. Compute new search direction
  %
  if abs(d3) < -SIG*d0 ...
...
%         disp('Failed twice.');
    ...
    if i > abs(len)	% or we ran out of time, so we give up
%         disp('Ran out of time.');
    ...
    x3 = 1/(1-d0);
    ls_failed = 1;  % flag search failed
  end
end
% fprintf('\n');